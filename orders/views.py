...
def orders_list(request):
    orders = Order.objects.filter(user=request.user)  # Assuming you have a user associated with the request
    context={
        'orders': orders
    }
...
def order_detail(request, order_id):
    ...
    refunds = RefundRequest.objects.filter(order=order) #get refund history for this order
